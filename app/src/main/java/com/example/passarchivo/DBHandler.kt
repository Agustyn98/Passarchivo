...
class DBHandler(context: Context?) : SQLiteOpenHelper(context, "passarchivo", null, 1) {
    ...
    fun getAllAccountsById(idCategory: Int, sortByNewest: Boolean = true): ArrayList<Account> {
        val accountArray = ArrayList<Account>()
        val db = this.readableDatabase
        var queryString = if (sortByNewest)
            "SELECT * FROM $tableNameAccount WHERE $COLUMN_ID_CATEGORY = $idCategory ORDER BY $COLUMN_ID DESC"
        else
    ...
    fun getAllAccounts(sortByNewest: Boolean = true): ArrayList<Account> {
        val accountArray = ArrayList<Account>()
        val db = this.readableDatabase

        val queryString = if (sortByNewest)
            "SELECT * FROM $tableNameAccount ORDER BY $COLUMN_ID DESC";
        else
            "SELECT * FROM $tableNameAccount"
    ...
    fun searchAccounts(search: kotlin.String): ArrayList<Account> {
        ...
        val db = this.readableDatabase
        val queryString =
            "SELECT * FROM $tableNameAccount WHERE $COLUMN_NAMEACCOUNT LIKE '%$search%' OR $COLUMN_EMAIL LIKE '%$search%' OR $COLUMN_USERNAME LIKE '%$search%' OR $COLUMN_NOTE LIKE '%$search%' OR $COLUMN_CUSTOM_NAME LIKE '%$search%';"
